...
function SendMoney() {
  return (
    <div className=" flex row cursor-pointer items-center gap-[10px] pl-[14px] bg-mustardyellow w-[139px] h-[35px] ">
      ...
      <p className="font-opensans text-[12px] text-white font-medium ">
        Send Money
      </p>
    </div>
  );
...
function Transfer({ onClose }) {
  ...
  if (!formData.senders_account || !formData.receiver_account || !formData.transfer_amount) {
    setResponse({
      status: "error",
  ...
  // Format data for API
  const transferData = {
    senders_account: formData.senders_account.trim(),
    receiver_account: formData.receiver_account.trim(),
    transfer_amount: Number(formData.transfer_amount),
    transfer_description: formData.transfer_description?.trim() || ""
  };
  ...
  try {
    ...
    setResponse({
      status: "error",
      error: error.response?.data?.error || "Transaction failed. Please try again."
    });
  } finally {
    setLoading(false);
  }
  ...
  return (
    <div className="flex-col flex items-center bg-white pb-[80px] w-[390px] h-[screen] shadow-lg">
      ...
      <form
        ...
        className="self-start w-[350px] px-[20px] flex flex-col gap-[14px]"
      >
        ...
        <div className="flex flex-col gap-[8px]">
          ...
          <input
            ...
            value={uiFormData.transfer_description}
            onChange={handleChange}
