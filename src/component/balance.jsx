...
function Balance() {
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      ...
      try {
        const response = await axios.get(
          "https://epaydatabase.onrender.com/account/balance",
          {
            params: {
    ...
    const intervalId = setInterval(fetchBalance, 45000);
  ...
  if (loading) {
    return (
      <div className="flex-col flex gap-[8px] h-[73px] w-[185px]">
        <p className="opensans font-thin text-[14px] text-white">
          Total Balance
        </p>
  ...
  return (
    <div className="flex-col flex gap-[8px]  w-[185px]">
      <p className="opensans font-medium text-[17px] text-white">
        {" "}
        Hi, {firstname}
      </p>
